...
function parseArguments(values) {
  ...
  return { image, project: resolve(project) };
}
...
function buildFiles(image, version, checkHosts) {
...
/** Executa o gerador. */
async function main() {
  const { image, project } = parseArguments(process.argv.slice(2));
  ...
  const files = buildFiles(image, version, checkHosts);
